...
class LlamaAttention(nn.Module):
    def __init__(self,hidden_dim,max_len,num_heads,num_kv_heads,dropout_rate,base=10000):
        super(self,LlamaAttention).__init__()
        ...
        self.query_net = nn.Linear(self.d_model,self.d_model,bias=True )
        self.value_net = nn.Linear(self.d_model,self.num_kv_heads*self.hidden_dim ,bias=True )
        self.key_net = nn.Linear(self.d_model,self.num_kv_heads*self.hidden_dim ,bias=True )
        self.output_net = nn.Linear(self.d_model,self.d_model ,bias=True ) 
        self.RopeEmb = RotaryEmbedding(self.hidden_dim,max_len,base)


    def forward(self,x,attention_mask):
        batch_size ,seq_len = x.shape()
        query_states = self.query_net(x)
        key_states = self.key_net(x)
        value_states = self.value_net(x)

        query_states = query_states.view(batch_size,seq_len,-1,self.hidden_dim).transpose(1,2)

        key_states = key_states.view(batch_size,seq_len,-1,self.hidden_dim).transpose(1,2)
        value_states = value_states.view(batch_size,seq_len,-1,self.hidden_dim).transpose(1,2)

        sin,cos = self.RopeEmb(key_states,seq_len)
        key_emb_states = get_rotary_pos_emb(key_states,sin,cos)
        value_emb_states = get_rotary_pos_emb(value_states,sin,cos)

        key_states = key_emb_states.unsqueeze(2).expand(batch_size,self.num_kv_heads,self.num_groups,seq_len,self.hidden_dim).reshape(batch_size,self.num_kv_heads*self.num_groups,seq_len,self.hidden_dim)
        value_states = value_emb_states.unsqueeze(2).expand(batch_size,self.num_kv_heads,self.num_groups,seq_len,self.hidden_dim).reshape(batch_size,self.num_kv_heads*self.num_groups,seq_len,self.hidden_dim)
        attention_weight = torch.matmul(query_states,key_states.transpose(-1,-2))
...
class LlamaAttention(nn.Module):
    ...
    def forward(self,query:torch.Tensor,attention_mask):
        ...
        Value = get_rotary_pos_emb(Key,sin,cos)
...
class  QwenDecoderlayer(nn.Module):
    def __init__(self,layer_id,hidden_dim,max_len,num_heads,num_kv_heads,num_expert,expert_intermediate_size,
        ...
        # 2. Attention
        self.Attention = LlamaAttention(self.hidden_dim,self.num_kv_heads,self.num_heads,self.max_len,self.dropout_rate,self.base)
        # 3. RMSNorm
