...
class QWen2Attention(nn.Module):
    ...
    def forward(self,query:torch.Tensor,attention_mask):
        ...
        Key = Key + get_rotary_pos_emb(Key,sin,cos)
        Value = Value + get_rotary_pos_emb(Key,sin,cos)
        ...
        # 计算注意力输出；[batch_zise,num_heads,seq_len,seq_len] *[batch_zise,num_heads,hidden_dim,seq_len]---> [batch_zise,num_heads,seq_len,hidden_dim]
...
class MOE(nn.Module):
    ...
    def _calculate_aux_loss(self, gate_logits, expert_mask):
        prob = torch.softmax(gate_logits, dim=-1)
        expert_usage = expert_mask.sum(dim=0).float()  # [E]
        uniform = torch.ones_like(expert_usage) / self.num_experts
        return torch.sum((expert_usage - uniform) ** 2)

# 添加辅助损失
## 目的是为了让每个专家网络都能够被使用；
def load_aux_loss_func(gate_logits,num_experts,top_k,attention_mask,) :
    if gate_logits is None or not isinstance(gate_logits, tuple):
        return 0
    # 将所有层的gate_logits合并
    ## 合并专家网络；gate_logits:[num_hidden_layers,batch_size*seq_len,num_experts] ----> [num_hidden_layers*batch_size*seq_len,num_experts]
    concatenated_gate_logits = torch.cat([layer_gate for layer_gate in gate_logits], dim=0)
    # 通过 softmax 计算每个 token 对专家的选择概率
    ## [num_hidden_layers*batch_size*seq_len,num_experts] --->  [num_hidden_layers*batch_size*seq_len,num_experts]
    routing_weights = torch.nn.functional.softmax(concatenated_gate_logits, dim=-1)
    ##  [num_hidden_layers*batch_size*seq_len,num_experts]-->[num_hidden_layers*batch_size*seq_len,topk]
    _, selected_experts = torch.topk(routing_weights, top_k, dim=-1)
    # 标记每个 token 实际使用的 K 个专家
    ## [num_hidden_layers*batch_size*seq_len,topk] -->[num_hidden_layers*batch_size*seq_len,top_k,num_experts]
    ...
    if attention_mask is None:
        ...
        # 统计所有专家在所有token 中实际被选中的次数，并归一化为比例
        ## [batch_size,seq_len] -->expand : [num_hidden_layers,batch_size,seq_len,top_k,num_experts]--> reshape:[num_hidden_layers*batch_size*seq_len,top_k,num_experts]
        expert_attention_mask = (
            attention_mask[None, :, :, None, None]
            .expand((num_hidden_layers, batch_size, sequence_length, top_k, num_experts))
            .reshape(-1, top_k, num_experts))

        ## [num_hidden_layers * batch_size * sequence_length, num_experts]* [num_hidden_layers * batch_size * sequence_length, num_experts]--->[num_hidden_layers * batch_size * sequence_length, num_experts]
        ## [num_hidden_layers * batch_size * sequence_length, num_experts]/[num_experts] --->[num_experts]
        tokens_per_expert = torch.sum(expert_mask.float() * expert_attention_mask, dim=0) / (torch.sum(expert_attention_mask, dim=0)+ 1e-6)

        # 统计所有专家在所有token 中理论被选中的次数，并归一化为比例
        # [batch_size,seq_len] -->expand : [num_hidden_layers,batch_size,seq_len,num_experts]--> reshape:[num_hidden_layers*batch_size*seq_len,num_experts]
        ...
        # [num_hidden_layers*batch_size*seq_len,num_experts] * [num_hidden_layers*batch_size*seq_len,num_experts] ---> [num_hidden_layers*batch_size*seq_len,num_experts]
        # [num_hidden_layers*batch_size*seq_len,num_experts] / [num_experts] --->[num_experts]
    ...
    ## 点积计算两个分布的差异
    # unsqueeze:[1,num_experts]---->[num_experts] * [1,num_experts] ==[num_experts,] * [1,num_experts]--->sum:[1,num_experts] --->标量
...
class  QwenDecoderlayer(nn.Module):
    def __init__(self,layer_id,hidden_dim,max_len,num_heads,num_kv_heads,num_expert,expert_intermediate_size,
        ...
        # 4. MOE
        self.mlp = MOE(self.hidden_dim,self.num_expert,self.expert_intermediate_size,self.topk,self.shared_expert_intermediate_size,self.activation_func,self.norm_topk)
    ...
    def forward(self,hidden_states,attention_mask,output_attention):
        ...
        hidden_states,gate_output = self.mlp(hidden_states)
        hidden_states = res + hidden_states
...
class QWENModel_MOE(nn.Module):
    def __init__(self,hidden_dim,max_len,num_heads,num_kv_heads,num_expert,expert_intermediate_size,
                 topk,shared_expert_intermediate_size,activation_func,norm_topk,dropout_rate,eps,base,
                 mode,vocab_size,num_hidden_layers,aux_loss_coef):
        super(QWENModel_MOE,self).__init__()
    ...
    def forward(self,input_ids,targets):
        ...
        for decoder_layer in self.decoder_layer_list:
            hidden_states,gate_logits = decoder_layer(hidden_states,attention_mask,self.output_attention)
            gate_logits_list.append(gate_logits)
        self.aux_loss =sum(load_aux_loss_func(gate_logits_list,self.num_expert,self.topk,attention_mask,))
        hidden_states = self.RMSNorm(hidden_states)
        if targets:# 下一个词
            logits = self.output(hidden_states)
            self.last_loss = F.cross_entropy(logits.view(-1, logits.size(-1)), 
                                             targets.view(-1), 
                                             ignore_index=-1) + self.aux_loss_coef*self.aux_loss
        else:
